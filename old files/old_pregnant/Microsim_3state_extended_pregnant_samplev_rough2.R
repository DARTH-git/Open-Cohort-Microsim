...
m.M <- rbindlist(list(m.M), use.names = F)
...
for (t in 1:n.t) {
    ...
    new_babies_m.M <- as.data.frame(matrix(NA, nrow=n.babies, ncol=n.t+1))
    ...
    # m.M <- as.data.frame(rbindlist(list(m.M, new_babies_m.M), use.names = F))
    m.M <- rbindlist(list(m.M, new_babies_m.M), use.names = F)
    }
